...
contract PumpFactoryNative {
    ...
    function createToken(
        ...
        require(msg.value == createFee + initialETH);
        ...
        (address _token0, address _token1) = address(newtoken) < address(weth) ? (address(newtoken), address(weth)) : (address(weth), address(newtoken));
        (uint256 _tk0AmountToMint, uint256 _tk1AmountToMint) = address(newtoken) < address(weth) ? (INITIALTOKEN, initialETH) : (initialETH, INITIALTOKEN);
        ...
        v3posManager.mint{value: initialETH}(params);
